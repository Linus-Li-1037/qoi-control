...
template<class T>
bool halfing_l2_error_V_TOT_uniform(const T * Vx, const T * Vy, const T * Vz, size_t n, const std::vector<unsigned char>& mask, const double tau, std::vector<double>& ebs){
	double eb_Vx = ebs[0];
	double eb_Vy = ebs[1];
	double eb_Vz = ebs[2];
	...
	max_value = print_RMSE(names[0], error_est_V_TOT);
	// std::cout << names[0] << ": max estimated error = " << max_value << ", index = " << max_index << std::endl;
	// estimate error bound based on maximal errors
	if(max_value > tau){
		// estimate
		long double estimate_error = max_value;
		double eb_Vx = ebs[0];
		double eb_Vy = ebs[1];
		double eb_Vz = ebs[2];
		std::vector<double> tmp_error_est_V_TOT(n, 0);
		while(estimate_error > tau){
    		// std::cout << "uniform decrease\n";
			eb_Vx = eb_Vx / 1.5;
			eb_Vy = eb_Vy / 1.5;
			...
			for(int i=0; i<n; i++){
				...
				double V_TOT = sqrt(V_TOT_2);
				// print_error("V_TOT", V_TOT, V_TOT_ori[i], e_V_TOT);
				tmp_error_est_V_TOT[i] = e_V_TOT;
			}
			estimate_error = print_RMSE(names[0], tmp_error_est_V_TOT);
		}
		ebs[0] = eb_Vx;
		ebs[1] = eb_Vy;
		ebs[2] = eb_Vz;
...
int main(int argc, char ** argv){
	...
	long double max_act_RMSE = 0, max_est_RMSE = 0;
	long double max_act_NRMSE = 0, max_est_NRMSE = 0;
    ...
    while((!tolerance_met) && (iter < max_iter)){
	    ...
	    for(int i=0; i<n_variable; i++){
			...
			// reconstruct_time += (double)(reconstruct_end.tv_sec - reconstruct_start.tv_sec) + (double)(reconstruct_end.tv_nsec - reconstruct_start.tv_nsec)/(double)1000000000;
	    ...
	    // MDR::print_vec(ebs);
	    tolerance_met = halfing_l2_error_V_TOT_uniform(Vx_dec, Vy_dec, Vz_dec, num_elements, mask, tau, ebs);
		// std::cout << "iter" << iter << ": The new ebs are:" << std::endl;
	    // MDR::print_vec(ebs);
	    // std::cout << names[0] << " requested error = " << tau << std::endl;
	    max_act_error = print_max_abs(names[0] + " error", error_V_TOT);
		max_act_RMSE = print_RMSE(names[0] + " error", error_V_TOT);
		max_act_NRMSE = print_NRMSE(names[0] + " error", error_V_TOT, compute_value_range(V_TOT));
	    max_est_error = print_max_abs(names[0] + " error_est", error_est_V_TOT);
		max_est_RMSE = print_RMSE(names[0] + " error_est", error_est_V_TOT);  
		max_est_NRMSE = print_NRMSE(names[0] + " error_est", error_est_V_TOT, compute_value_range(V_TOT));
    }
	...
	std::cout << "max_est_error / max_act_error = " << max_est_error / max_act_error << std::endl;
	std::cout << "max_est_RMSE = " << max_est_RMSE << std::endl;
	std::cout << "max_act_RMSE = " << max_act_RMSE << std::endl;
	std::cout << "max_est_RMSE / max_act_RMSE = " << max_est_RMSE / max_act_RMSE << std::endl;
	std::cout << "max_est_NRMSE = " << max_est_NRMSE << std::endl;
	std::cout << "max_act_NRMSE = " << max_act_NRMSE << std::endl;
	std::cout << "max_est_NRMSE / max_act_NRMSE = " << max_est_NRMSE / max_act_NRMSE << std::endl;
	...
	std::cout << "mask_size: " << mask_file_size << std::endl;
