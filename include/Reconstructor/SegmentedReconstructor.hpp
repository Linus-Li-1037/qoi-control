#ifndef _MDR_SEGMENTED_RECONSTRUCTOR_HPP
#define _MDR_SEGMENTED_RECONSTRUCTOR_HPP
...
namespace MDR {
    ...
    class SegmentedReconstructor : public concepts::ReconstructorInterface<T> {
    public:
        SegmentedReconstructor(Decomposer decomposer, Interleaver interleaver, Encoder encoder, Compressor compressor, SizeInterpreter interpreter, Retriever retriever)
        ...
        T * reconstruct(uint32_t requested_size, int max_level=-1){
            // Timer timer;
            // timer.start();
            ...
            if(max_level == -1 || (max_level >= level_num_bitplanes.size())){
                auto retrieve_sizes = interpreter.interpret_retrieve_size(level_sizes, level_errors, requested_size, error_bound, level_num_bitplanes);
                // retrieve data
                ...
                auto retrieve_sizes = interpreter.interpret_retrieve_size(tmp_level_sizes, tmp_level_errors, requested_size, error_bound, tmp_level_num_bitplanes);
        ...
        T * progressive_reconstruct(uint32_t requested_size, int max_level=-1){
            // std::vector<T> cur_data(data);
            reconstruct(requested_size, max_level);
